...
try:
    ...
    print(f"  Runway polygons loaded as full forbidden buffer ({len(gdf_runways_3857)} remaining)")
...
print("Assigning Clutter to Existing Sites...")
if HAS_FIONA:
    try:
        with fiona.open(CLUTTER_PATH) as src:
            pass # We don't read all into memory, takes too long. Just read bounds if needed
    except Exception as e:
        print(f"Failed to read clutter shapefile: {e}")
        HAS_FIONA = False
else:
    print("Fiona not installed, defaulting to 600m clutter radius.")

def get_clutter_radius(lon, lat):
    if global_clutter_gdf.empty:
        return DEFAULT_RADIUS
    
    pt = Point(lon, lat)
    # Filter by bounding box first for speed
    possible_matches = global_clutter_gdf.cx[lon-0.01:lon+0.01, lat-0.01:lat+0.01]
    
    for _, row in possible_matches.iterrows():
        if row.geometry.contains(pt):
            c_type = str(row.get('CLUTTER', '')).strip().upper()
            return CLUTTER_RADII.get(c_type, DEFAULT_RADIUS)
...
for k, df in raw_data.items():
    if 'RSRQ' in k:
        if len(df) > 0:
            col_name = 'RSRQ(All MRs) (dB)' if 'RSRQ(All MRs) (dB)' in df.columns else 'RSRQ'
            v_min = df[col_name].min()
            v_max = df[col_name].max()
...
for apt in airports:
    ...
    def get_isd_min(pt_3857):
        return 1300  # Minimum 1300m site-to-site distance (MUST requirement)
    ...
    for metric in ['RSRP', 'RSRQ']:
        for env in ['Combine', 'Indoor']:
            for src in ['MR', 'MDT']:
                ...
                df = raw_data[key]
                if src in ['MR', 'MDT']:
                    df = df.rename(columns={f'{metric}(All MRs) (dBm)' if metric == 'RSRP' else f'{metric}(All MRs) (dB)': metric})
                df_filtered = df[
    ...
    try:
        all_rsrp_dfs = [processed_gdfs[k] for k in processed_gdfs if 'RSRP_Combine' in k and processed_gdfs[k] is not None]
        if all_rsrp_dfs:
            ...
            if len(gdf_in) > 0:
                ...
                print(f"DEBUG: airport_poly_3857 bounds={airport_poly_3857.bounds}")
                print(f"DEBUG: gdf_in bounds={gdf_in.total_bounds}")
                print(f"DEBUG: airport_poly_3857 bounds={airport_poly_3857.bounds}")
                print(f"DEBUG: gdf_in bounds={gdf_in.total_bounds}")

                # Filter to airport polygon (both in 3857)
                gdf_in = gdf_in[gdf_in.geometry.within(airport_poly_3857)].copy()

                gdf_3857 = gdf_in.copy()

                # Calculation happens inside buffered polygon
                airport_poly_3857_buffered = airport_poly_3857.buffer(300)
                pts_inside = gdf_in[gdf_in.geometry.within(airport_poly_3857_buffered)]
                bad_spots = pts_inside[pts_inside['RSRP'] < -105].copy()
                total_pts_in_airport = len(pts_inside)
                
                print(f"DEBUG: total={total_pts_in_airport} bad={len(bad_spots)}")
                if total_pts_in_airport > 0 and len(bad_spots) > 0:
                    ...
                    iteration = 0
                    previous_uncovered_len = -1
                    
                    while len(uncovered_bad_spots) > 0 and iteration < 2000:
                        previous_uncovered_len = len(uncovered_bad_spots)
                        print(f"  Iteration {iteration}, Remaining Bad Spots: {len(uncovered_bad_spots)}")
                        iteration += 1
                        
                        # Focus heavily on the lowest 10% RSRP bins
                        q10 = uncovered_bad_spots['RSRP'].quantile(0.10)
                        worst_spots = uncovered_bad_spots[uncovered_bad_spots['RSRP'] <= q10]
                        if len(worst_spots) == 0: worst_spots = uncovered_bad_spots
                        
                        # Use KMeans to find the center of the worst spots (K=1 for the highest density area)
                        from sklearn.cluster import KMeans
                        coords = np.array([(geom.centroid.x, geom.centroid.y) for geom in worst_spots.geometry])
                        if len(coords) >= 3:
                            ...
                            centroid_3857 = Point(cluster_center[0], cluster_center[1])
                            
                            # Find the nearest actual bad spot to the centroid (K-Medoids style)
                            dists = [Point(c[0], c[1]).distance(centroid_3857) for c in coords[kmeans.labels_ == largest_cluster_idx]]
                            best_idx = worst_spots[kmeans.labels_ == largest_cluster_idx].index[np.argmin(dists)]
                        else:
                            best_idx = worst_spots.index[0]
                            centroid_3857 = Point(coords[0][0], coords[0][1])
                            
                        lon_c, lat_c = transformer_to_4326.transform(centroid_3857.x, centroid_3857.y)
                        ...
                        for s_key, s_info in current_sites_info.items():
                            ...
                            # Expand search boundary to 1300m to eliminate dead zones where new sites are blocked
                            if dist < closest_dist and dist <= 1300:
                                closest_dist = dist
                        ...
                        if closest_site_key is not None:
                            ...
                            if site_type == 'IBS' and closest_dist <= radius_m:
                                if is_valid_azimuth(bearing_to_cluster, s_info['azimuths'], 90):
                                    ...
                                    eff_radius = max(radius_m, closest_dist + 50)
                                    poly_calc = get_sector_polygon(s_info['x'], s_info['y'], s_info['lat'], bearing_to_cluster, radius_m=eff_radius, angle_deg=65)
                                    poly_viz = get_sector_polygon(s_info['x'], s_info['y'], s_info['lat'], bearing_to_cluster, radius_m=300, angle_deg=65)
                            ...
                            if not placed and site_type in ['MACRO', 'IBC2M'] and closest_dist <= 1300:
                                ...
                                if len(s_info['azimuths']) < max_sec:
                                    if is_valid_azimuth(bearing_to_cluster, s_info['azimuths'], 90):
                                        ...
                                        # Authentic pathloss coverage evaluation footprint
                                        eff_radius = max(radius_m, closest_dist + 50)
                                        poly_calc = get_sector_polygon(s_info['x'], s_info['y'], s_info['lat'], bearing_to_cluster, radius_m=eff_radius, angle_deg=65)
                                        poly_viz = get_sector_polygon(s_info['x'], s_info['y'], s_info['lat'], bearing_to_cluster, radius_m=300, angle_deg=65)
                                        global_additional.append(poly_viz)
                        ...
                        if not placed:
                            ...
                            tlp_pid = 'N/A'
                            tlp_pname = 'N/A'
                            airport_runway = gdf_runways_3857[gdf_runways_3857.intersects(airport_poly_3857)]

                            def is_in_runway(pt):
                                if airport_runway.empty: return False
                                return any(poly.buffer(5).contains(pt) for poly in airport_runway.geometry)
                            ...
                            if ns_pt is None:
                                if is_in_runway(centroid_3857):
                                    if not airport_runway.empty:
                                        import math
                                        from shapely.ops import nearest_points
                                        geom = airport_runway.geometry.unary_union
                                        p1, p2 = nearest_points(geom.boundary, centroid_3857)
                                        if geom.contains(centroid_3857):
                                            vx, vy = p1.x - centroid_3857.x, p1.y - centroid_3857.y
                                        else:
                                            vx, vy = centroid_3857.x - p1.x, centroid_3857.y - p1.y
                                        
                                        dist = math.hypot(vx, vy)
                                        if dist > 0:
                                            ns_pt = Point(p1.x + (vx/dist) * 100, p1.y + (vy/dist) * 100)
                                            # Loop to strictly ensure we pushed it out of any curved/thick boundaries
                                            safety = 0
                                            while is_in_runway(ns_pt) and safety < 10:
                                                ns_pt = Point(ns_pt.x + (vx/dist) * 50, ns_pt.y + (vy/dist) * 50)
                                                safety += 1
                                        else:
                                            ns_pt = Point(centroid_3857.x, centroid_3857.y + 100)
                                            safety = 0
                                            while is_in_runway(ns_pt) and safety < 10:
                                                ns_pt = Point(ns_pt.x, ns_pt.y + 50)
                                                safety += 1
                                    else:
                                        ns_pt = centroid_3857
                                else:
                            ...
                            if ns_pt is not None:
                                ...
                                for az in azs:
                                    ...
                                    poly_viz = get_sector_polygon(ns_pt.x, ns_pt.y, lat_ns, az, radius_m=300, angle_deg=65)
                                ...
                                # Strict exclusion zone to prevent over-allocation (consumes all nearby demand)
                                dists_to_new = np.array([geom.centroid.distance(ns_pt) for geom in uncovered_bad_spots.geometry])
                                uncovered_bad_spots = uncovered_bad_spots[dists_to_new > 1300]
                                placed = True
                        if not placed or len(uncovered_bad_spots) == previous_uncovered_len:
                            # Drop only the targeted spot to avoid infinite loop without pruning healthy adjacent spots
                            uncovered_bad_spots = uncovered_bad_spots.drop(best_idx)
    except Exception as e:
        import traceback
        tb = traceback.format_exc()
        print(tb)
        print(f"Error optimizing proposals for {name}: {e}")
    ...
    for dsrc, dname in [('Combine', 'Combine'), ('Indoor', 'Indoor')]:
        ...
        if combined_calc_poly and bad_count > 0:
            covered_bad = len(bad_spots_gdf[bad_spots_gdf.geometry.centroid.within(combined_calc_poly)])
    ...
    if 'ALL_COVERAGE_STATS' not in globals():
        global ALL_COVERAGE_STATS
        ALL_COVERAGE_STATS = {}
